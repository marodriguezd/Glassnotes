...
class NoteCard(CardWidget):
    ...
    def _setup_ui(self, title, preview, modified_time):
        ...
        self.delete_btn.setToolTip("Delete from system")
        ...
        # Unlink Button
        self.unlink_btn = TransparentToolButton(FIF.REMOVE, self)
        self.unlink_btn.setFixedSize(28, 28)
        self.unlink_btn.setToolTip("Remove from history (keep file)")
        self.unlink_btn.clicked.connect(self._on_unlink_clicked)
        # Custom style for gray hover effect
        self.unlink_btn.setStyleSheet("""
            TransparentToolButton {
                background: transparent;
                border: none;
                border-radius: 6px;
    ...
    def _on_unlink_clicked(self):
        self.unlinkRequested.emit(self.path)
...
class HubView(QWidget):
    ...
    def delete_note(self, path):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            if file_manager.delete_file(path):
                ...
                # Notify main window to close any open tabs with this file
    ...
    def unlink_note(self, path):
        """Remove note from app history without deleting file"""
        name = os.path.basename(path)
        reply = QMessageBox.question(
            self,
            "Remove from History",
    ...
    def update_cloud_list(self, drive_notes, skip_store=False):
        ...
        if not notes:
            empty = QLabel(
                ...
                if self._cloud_notes
                else "Connect Google Drive to sync notes"
            )
    ...
    def update_recent_list(self, recent_files, skip_store=False):
        ...
        if not notes:
            ...
            for path in notes:
                ...
                card.unlinkRequested.connect(self.unlink_note)
